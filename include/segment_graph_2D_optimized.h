 ...
 */
 ...
 //                    if (candidate_id == 0) continue;
   ...
   virtual tableint mutuallyConnectNewElementLevel0(
       ...
       while (queue_closest.size()) {
         ...
         if (return_list.size() >= Mcurmax ||
           ...
           if (batch_counter == 0) {
             ...
             // for avoid duplicates
             for (auto candidate_idx = 0; candidate_idx < return_list.size(); candidate_idx++) {
               if ((candidate_idx >= Mcurmax / 2) || return_list[candidate_idx].first != return_list[candidate_idx - 1].first) {
                 selectedNeighbors.push_back(return_list[candidate_idx].second);
               }
             }
           }
     ...
     for (size_t idx = 0; idx < selectedNeighbors.size(); idx++) {
       // only keep half m, for removing duplicates, this is same as original HNSW
       // if (idx > maxM0_ / 2) {
       //   break;
       // }
 ...
 class IndexSegmentGraph2D : public BaseIndex {
  public:
   ...
   BaseIndex::IndexParams index_params_;
   SegmentGraph2DHNSW<float> *hnsw = nullptr;
   unsigned index_k = 0;
   ...
   void buildIndex(const IndexParams *index_params) override {
     ...
     index_params_.K = index_params->K;
     index_params_.ef_max = index_params->ef_max;
     index_params_.ef_construction = index_params->ef_construction;
     // build HNSW
 ...
 #pragma omp parallel for schedule(monotonic : dynamic)
  ...
  vector<int> rangeFilteringSearchOutBound(
     ...
     std::priority_queue<pair<float, int>> top_candidates;
     std::priority_queue<pair<float, int>> candidate_set;
     int index_k_ = index_params_.K;
     int search_ef_ = search_params->search_ef;
     ...
     while (!candidate_set.empty()) {
       ...
       for (auto batch_it : neighbor_iterators) {
         ...
         for (auto candidate_id : *batch_it) {
           ...
           if (visited_nn_num > 2 * index_k_) break;
           if (!(visited_array[candidate_id] == visited_array_tag)) {
             visited_array[candidate_id] = visited_array_tag;
            ...
            float dist = fstdistfunc_(query,
             ...
             if (top_candidates.size() < search_ef_ || lower_bound > dist) {
     ...
     search_info->total_comparison += num_comparison;
   ...
   void save(const string &save_path) override {
     std::ofstream output(save_path, std::ios::binary);
     unsigned counter = 0;
     base_hnsw::writeBinaryPOD(output, index_params_.K);
     base_hnsw::writeBinaryPOD(output, index_params_.ef_max);
     base_hnsw::writeBinaryPOD(output, index_params_.ef_construction);
   ...
   void load(const string &load_path) override {
     std::ifstream input(load_path, std::ios::binary);
     if (!input.is_open()) throw std::runtime_error("Cannot open file");
     directed_indexed_arr.clear();
     directed_indexed_arr.resize(data_wrapper->data_size);
     base_hnsw::readBinaryPOD(input, index_params_.K);
     base_hnsw::readBinaryPOD(input, index_params_.ef_max);
     base_hnsw::readBinaryPOD(input, index_params_.ef_construction);
     int forward_num;
     int reverse_num;
     int batch_num;
     int start_pos;
